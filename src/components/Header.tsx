...
export const Header = ({ showProfile = false }: HeaderProps) => {
  ...
  return (
    <>
      ...
      {showProfile && user && (
        <div className="absolute top-4 right-4 md:top-6 md:right-6 flex items-center gap-4 md:gap-6 bg-white/80 backdrop-blur px-4 py-2 rounded-xl shadow-sm border border-neutral-200">
          <div className="flex flex-col items-end hidden sm:flex">
            <span className="font-oxanium font-bold text-dark text-sm">{user.name || user.username}</span>
            <span className="font-oxanium text-xs text-neutral-500">Peserta</span>
          </div>
          <div className="w-10 h-10 md:w-10 md:h-10 rounded-full bg-primary/20 text-primary flex items-center justify-center font-bold shadow-sm">
